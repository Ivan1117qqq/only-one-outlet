...
export const remaining = (s: GameState): number => Math.max(0, C.duration - s.elapsed);
...
export function startUpload(s: GameState, id: number): boolean {
  const t = s.tasks.find(t => t.id === id);
  if (!isActive(s) || s.battery <= 0 || s.uploadingId !== null || !t || t.status !== 'ready') return false;
...
function finishTask(s: GameState, t: Task, outcome: Outcome): void {
  ...
  s.history.push({ id: t.id, name: t.name, outcome, score, at: s.elapsed,
...
export function step(s: GameState, seconds: number): void {
  ...
  s.battery = Math.max(0, Math.min(100, s.battery + baseRate * dt - C.uploadDrainPerSecond * uploadTime));
  ...
  s.elapsed = Math.min(C.duration, end);
  ...
  if (remaining(s) < 1e-8) {
    s.elapsed = C.duration;
    for (const t of [...s.tasks]) finishTask(s, t, 'unfinished');
    s.phase = 'ended'; s.paused = false; return;
  }
  const template = generateTask(s.generator, s.elapsed, s.tasks.length, s.tasks.some(t => t.urgent));
  if (template) {
    s.tasks.push({ id: s.generator.count, name: template.name, reward: template.reward, urgent: template.urgent,
      arrivedAt: s.elapsed, dueAt: s.elapsed + template.deadline,
