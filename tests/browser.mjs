...
try {
  ...
  assert.equal(await page.locator('#phone-send').isDisabled(), true);
  await page.locator('#phone-delivery').selectOption('1');
  await page.clock.runFor(100);
  assert.equal(await page.locator('#phone-delivery').inputValue(), '1');
  await page.setViewportSize({ width: 320, height: 740 });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
  await page.screenshot({ path: 'test-results/phone-delivery-mobile.png', fullPage: true });
  await page.setViewportSize({ width: 1200, height: 1000 });
  await page.locator('#phone-send').click();
  assert.equal(await page.locator('#phone-delivery').inputValue(), '');
  ...
  assert.match(await page.locator('.terms:visible').first().innerText(), /截止時間實際延後/);
