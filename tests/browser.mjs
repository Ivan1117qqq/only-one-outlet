...
try {
  ...
  assert.match(await page.locator('[data-field="reward"]').first().innerText(), /52/);
  await page.clock.runFor(3800);
  await page.locator('[data-action="upload"]:visible').click();
  await page.clock.runFor(3100);
  assert.equal(await page.locator('#score').innerText(), '52');
  ...
  await page.locator('[data-action="cancel-call"]:visible').click();
  await extension.first().click();
  await page.keyboard.press('3');
