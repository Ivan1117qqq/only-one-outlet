...
- 預設為「再一次」雙輪模式，每輪 100 秒，第一輪回覆解鎖第二輪重點版與電話延期；`src/encore.ts` 管理共用時程與內情，數值在 `src/config.ts` 的 ENCORE。180 秒核心僅保留作回歸測試。
- 交件／逾期／放棄／下班漏件共用唯一結案入口，避免重複計分。暫停必須停止全部模擬，包含通話；通話與上傳互斥。兩輪通知時程不受玩家操作影響。
